...
export default function CartItem (props) {
     const cartContent = props.cartItems.map((item, idx) => 
          <div key={item.id} className='flex flex-row justify-between mt-5 w-full cursor-pointer' id={idx}>
     ...
     return (
          <section className='flex'>
               {props.cartItems.length >= 1 && <div  className='flex flex-col px-5 justify-between w-full h-full pb-10'>
                    {cartContent}
                    <button className='rounded-lg px-8 py-2 text-[#ffffff] mt-10' onClick={props.check}>Checkout</button>
               </div>
               }
